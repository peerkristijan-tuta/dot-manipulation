...
The converter method in the classes ExtravagantDisplay and Coordinator encode and decode alphabetical keys into and from numbers in the same way. Program I/O is executed by two instantiations of the IO thread class representing input and output respectively, and each instance runs a synchronized method called method which ensures that only one of the two can carry out operations at a time by invoking wait and notify. ExtravagantDisplay is executed when the program detects that it is run for the third time. Output history is stored as the encoded numbers in a text file (which also tracks run count) whilst the numbers are decoded to a specific dot location in its cluster.
